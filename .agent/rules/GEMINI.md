...
🛡️ JS & UI Safety (Global)
Dependency Check: Toda inicialização de bibliotecas externas (ex: new bootstrap.Modal, Swal.fire) DEVE ser envolvida em um check de segurança: if (typeof libraryName !== 'undefined').

Template Integrity: Arquivos .html em projetos de larga escala (como o Agendha) NUNCA devem ser órfãos. Sempre valide se o arquivo estende o template base correto (base.html ou similar).

Silent Fails: Prefira console.error informativos em vez de deixar o script quebrar silenciosamente, afetando outros componentes da página.
...
## 🧩 Workflow de Validação de Contexto
- **P0 Check:** Antes de gerar código, verifique se o agente `frontend-specialist` carregou as regras de `agendha-specific.md`.
- **Pre-flight Check:** Em cada `/orchestrate`, valide se as dependências (Bootstrap/CSS) estão acessíveis no arquivo que será modificado.

# Language Rules
1. You MUST always respond in Portuguese (Pt-BR), regardless of the input language.
2. Maintain technical terms in English when they are standard (e.g., "Frontend", "Deploy", "Framework"), but explain them if necessary.
