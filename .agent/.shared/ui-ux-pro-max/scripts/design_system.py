...
def format_markdown(design_system: dict) -> str:
    ...
    # Colors section
    lines.append("### Colors")
    lines.append(f"| Role | Hex |")  # noqa: F541
    ...
    if typography.get("css_import"):
        lines.append(f"- **CSS Import:**")  # noqa: F541
        lines.append(f"```css")  # noqa: F541
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append(f"  background: transparent;")  # noqa: F541
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    if style_results:
        ...
        style_name = style.get("Style Category", "")  # noqa: F841
        keywords = style.get("Keywords", "")
        best_for = style.get("Best For", "")  # noqa: F841
...
def _detect_page_type(context: str, style_results: list) -> str:
    ...
    if style_results:
        style_name = style_results[0].get("Style Category", "").lower()  # noqa: F841
